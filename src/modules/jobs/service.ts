...
export async function startJob(jobId: string, userId: string) {
  return await db.transaction(async (tx) => {
    ...
    if (job.status !== "scheduled") throw new Error("Job must be in scheduled status to start");
...
export async function completeJob(
  ...
  data: {
    ...
    additionalCharges: { description: string; quantity: number; unitPrice: number }[];
  },
) {
  return await db.transaction(async (tx) => {
    const [job] = await tx
    ...
    await tx
      .update(jobsTable)
      .set({ status: "completed", notes: data.notes ?? null, updatedAt: new Date() })
      .where(eq(jobsTable.id, jobId));
    ...
    for (const update of data.assetUpdates) {
      if (update.status) {
        await tx
          .update(assetsTable)
          .set({ status: update.status as any, lastServiceAt: new Date(), currentJobId: null })
    ...
    for (const item of jobItems) {
      ...
      invoiceItemsCreated.push({ id: iiId, description, quantity: item.quantity, unitPrice, total });
    }
...
export async function failJob(
  ...
  return await db.transaction(async (tx) => {
    ...
    if (!job) throw new Error("Job not found");
    if (job.status !== "in_progress") throw new Error("Job must be in progress to fail");
...
export async function returnAssetsFromJob(jobId: string, assetIds: string[], userId: string) {
  return await db.transaction(async (tx) => {
